## 🆕 (16/09/2026) CONTENT STUDIO AUTO-POSTER — direct publish to IG/FB via Meta API (parked — honest: not worth it yet)
**What:** A "Publish" button in Content Studio (studio.marble-art.co.il) that pushes a finished post — caption + hashtags + the Canva/rendered image — straight to Instagram + the Marble Art Sinks Facebook Page via the Meta Graph API (Instagram Content Publishing API), scheduled per the weekly-batch day plan. Removes the manual Canva→IG upload step entirely.
**Why PARKED, not built (honest call):** The Meta app-review gate is heavy — a Facebook App in Developer mode, `instagram_content_publish` + `pages_manage_posts` permissions, Business Verification, App Review with screencasts, and a Professional IG account linked to the Page (all already linked, which helps). The publishing API also can't post the AI *video/reel* pipeline cleanly and image posts must be a public URL (needs the image hosted, e.g. Cloudinary) — so it's a real backend build, not a button. At current volume (first reels just going live, posting a few times/week via Canva Pro), the manual Canva→IG flow costs minutes and gives a human eyeball on brand quality before anything goes public. ROI is negative until posting cadence is high enough that manual upload becomes the bottleneck.
**Scope when built:** (1) Meta App + Business Verification + App Review; (2) long-lived Page access token stored server-side (Vercel env, like the Anthropic key); (3) host the post image at a public URL (Cloudinary, reuse marble-art dqdku88vv); (4) new /api/publish route: create IG media container → poll status → publish; optional FB Page photo post in the same call; (5) a "Publish / Schedule" button + status in the batch view, tied to each day's slot. Keep the human-approve step (preview → confirm → publish), never fully silent.
**Effort:** big (mostly Meta review + token/hosting plumbing, not the code). **Value:** high once cadence is high; not now. **Trigger to build:** when Avshi is posting daily and the Canva→IG hop is the friction. Until then, Canva Pro → IG/FB stays the flow.
**Related shipped (16/09):** Content Studio Weekly Batch (5 photos → 5 varied posts/one click), Save-to-history (localStorage), weekly .ics calendar export, and per-post "צור וריאציה" — all live in the studio, no DB added.
...
- `[ ]` (27/08/2026) Auto-fill Nano Banana prompt per saved sketch — the prompt-builder pre-fills each sketch's prompt from its own spec (basin count, length, floor sloped/90°, stone), so a project yields its matched image set with zero manual writing (e.g. נשים·6 + גברים·4 × slope/90° = 4 aligned prompts). Interim/lightweight option: a copy-button prompt sheet or `.txt` export of the ready prompts. (Surfaced 27/08 after writing the 4 restroom prompts by hand.)
...
--- ADDED 06/07/2026 (Damari / Harel Sagron project — shop-drawing session) ---

CONTEXT: Ales was handed a raw AutoCAD design drawing (Harel Sagron architects) for the
Damari Center Dimona project — public restrooms, porcelain-granite trough sinks. Ales lacks
back-office/engineering: cannot turn AutoCAD into shop drawings, cannot price systematically,
chases the architect for details. Avshi/ARVO provided that layer as a teaser to demonstrate value
(no partnership demanded yet). Ales responded "תודה רבה מעריך את זה מאוד 🤝🙏" and acknowledged the
job has "הרבה ניואנסים" — value landed. Owner drives a Maybach; prestige job vs. serious architects.

DELIVERABLES PRODUCED THIS SESSION (all standalone HTML→PNG/PDF via wkhtmltoimage, ARVO-branded,
Hebrew RTL — NOT in the CRM yet):
- Two full shop drawings: women's 340cm (4 basins) + men's 320cm (4 basins). Each sheet:
  plan · section A-A · elevation · steel mounting detail · hidden-drain detail · fabrication notes.
- Standalone hidden-drain detail sheet.
- Seam-alignment method sheet (sink vs wall tile grid).
- Standalone siphon sheet (superseded by hidden-drain).
- ARVO-branded 2-render presentation document (terrazzo + black slate options).

KEY TECHNICAL FACTS LOCKED (corrected during session):
- Counters: women's 3400mm, men's 3200mm; both 4 basins × 600mm, 500mm deep, 250mm apron, wall-hung.
- Material: גרניט פורצלן (porcelain-granite) — architect's own label "כיור גרניט פורצלן". NOT Caesarstone.
  Two board colours: טרצו (light, flecked 120x120) + אריח שחור (smooth black slate 120x120).
- Build: 12mm double lamination (2×6mm).
- MOUNTING: continuous steel ledger + concealed cantilever arms + M12 chemical anchors; weight
  ~120kg women's / ~110kg men's; MUST confirm wall type (concrete/block = chemical anchor; drywall =
  steel backing between studs before closing wall). Structural engineer sign-off recommended.
- HIDDEN DRAIN (Ales's real method): porcelain cover cut from tile, floats on 4 feet (פקקים),
  flush with basin, 2-3mm perimeter gap all 4 sides → water into inner envelope (מעטפה) → bottle trap.
  Cover has lift-notches (rounded cut-out) on both short ends to finger-pull for cleaning. Fully hidden.
- SEAM ALIGNMENT: sink centreline + drain + faucet on one vertical line, ideally on a tile joint/centre;
  edges relate to horizontal courses; drain not on a seam. Water point set by plumber BEFORE tiling —
  coordinate centreline with plumber+architect early.

FUTURE CRM EMBEDDING IDEAS (how this becomes a repeatable capability, not one-off files):

[ ] 17. SHOP-DRAWING GENERATOR (CRM module): from a saved sketch's dimensions, auto-generate an
    ARVO-branded shop-drawing sheet (plan/section/elevation/mounting/drain/notes) as PNG+PDF, the way
    we did by hand today. Reuse the sketch data + the HTML→image pipeline concept. Per-project title block.
    This is the single biggest "wow" capability — turns any sketch into a professional submission.

[ ] 18. STANDARD DETAIL LIBRARY (CRM): reusable ARVO detail blocks — hidden-drain (4-feet cover),
    steel mounting detail, seam-alignment method, siphon options. Pick + drop into any drawing.
    The mounting detail also feeds parked idea (metal-hanger module).

[ ] 19. ARVO RENDER-PRESENTATION generator (CRM): plug 1-2 render images into the ARVO branded
    template → sendable PDF, with honesty label (הדמיה) auto-applied. (Did by hand today.) Ties to
    idea #14 (sketch→Nano Banana prompt) — sketch feeds the prompt, prompt feeds Nano, render feeds this.

[ ] 20. FIELD-SURVEY SHEET generator (CRM): blank ARVO form Ales fills on site (wall photo + tile size,
    grout width, course height, existing water/waste point) → feeds set-out before fabrication.

[ ] 21. "TAKE ANY INPUT → WORKING SKETCH + PRICE" pipeline (the pitch Avshi made to Ales): photo /
    video / AutoCAD / hand sketch → CRM builds a working sketch → porcelain sheet calc (how many sheets
    at Trabelsi price) → 10-minute professional price offer for any architect/big builder. This is the
    umbrella vision that ideas #14, #17, #19, #20 all serve.

BUSINESS FOLLOW-UPS (Avshi, not code):
[ ] Ales to confirm with architect: counter material — טרצו or אריח שחור? (decides porcelain order + render)

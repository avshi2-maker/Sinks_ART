...
export interface QuoteLineRow {
  ...
  unit_cost:        number;
  vat_applies:      boolean;
  line_total:       number;             // GENERATED column, read-only from app
  sink_id:          string | null;
...
/** A quote with its lines pre-loaded (used in detail views). */
export interface QuoteWithLines extends QuoteRow {
  lines: QuoteLineRow[];
}

/** Hebrew label for a status, used in badges. */
...
/** Status badge color class (Tailwind), used by UI components. */

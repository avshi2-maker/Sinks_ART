...
// Ales's mobile pricing screen. Reads job + media + active add-ons; he ticks/prices + breakdown + site photo + send.
...
function MediaItem({ asset }: { asset: RfqAsset }) {
  if (asset.kind === 'video') {
    return <video src={asset.url} controls className="w-full rounded-md border border-stone-200 bg-black" style={{ maxHeight: 220 }} />;
  }
  if (asset.kind === 'audio') {
    return (
      <div className="w-full border border-stone-200 rounded-md p-2 bg-stone-50">
        <div className="text-xs text-stone-500 mb-1">🎤 הקלטת לקוח</div>
        <audio src={asset.url} controls className="w-full" />
      </div>
    );
  }
  // image, sketch, file -> show as image (sketch is an svg/png URL)
  return (
    <a href={asset.url} target="_blank" rel="noopener noreferrer">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={asset.url} alt={asset.label || 'מדיה'} className="w-full rounded-md border border-stone-200 object-cover" style={{ maxHeight: 240 }} />
    </a>
  );
}
...
export default function AlesRfqForm({ rfq, options }: { rfq: RfqRow; options: OptionRow[] }) {
  ...
  const numInput = (label: string, val: number, set: (n: number) => void) => (
    <label className="block">
      ...
      <input type="number" value={val || ''} onChange={(e) => set(Number(e.target.value) || 0)} placeholder="0" className="w-full px-2 py-2 text-base border border-stone-300 rounded-md" dir="ltr" />
    </label>
  );

  return (
    <div className="max-w-md mx-auto px-3 py-4" dir="rtl">
      <div className="rounded-lg p-3 mb-3 flex items-center gap-3" style={{ background: ARVO_DARK }}>
        <div className="w-9 h-9 rounded-md flex items-center justify-center font-bold" style={{ background: ARVO_GOLD, color: ARVO_DARK }}>A</div>
        <div>
          <div className="text-sm font-medium text-white">תמחור · {rfq.title_he}</div>
          <div className="text-xs" style={{ color: ARVO_GOLD }}>{rfq.project_ref || ''}{rfq.customer_hint ? ' · ' + rfq.customer_hint : ''}</div>
        </div>
      </div>

      <div className="bg-stone-50 border border-stone-200 rounded-lg p-3 mb-3 text-sm text-stone-700">
        ...
        {spec.notes && (<div className="text-xs text-stone-500 mt-1">{spec.notes}</div>)}
      </div>
          ...
          <div className="space-y-2">
            {options.map((o) => (
              <div key={o.id} className="flex items-center gap-2">
                <input type="checkbox" checked={!!picked[o.id]} onChange={(e) => setPicked((p) => ({ ...p, [o.id]: e.target.checked }))} className="w-5 h-5" />
                <span className="flex-1 text-sm text-stone-700">{o.name_he}</span>
                {picked[o.id] && (<input type="number" value={addonPrice[o.id] || ''} onChange={(e) => setAddonPrice((p) => ({ ...p, [o.id]: Number(e.target.value) || 0 }))} placeholder="₪" className="w-20 px-2 py-1 text-sm border border-stone-300 rounded text-center" dir="ltr" />)}
              </div>
            ))}
          </div>
        </div>
      ...
      <label className="block mb-3">
        ...
        <input value={deliveryDays} onChange={(e) => setDeliveryDays(e.target.value)} placeholder="למשל: 14 ימי עבודה" className="w-full px-2 py-2 text-sm border border-stone-300 rounded-md" dir="rtl" />
      </label>

      <label className="block mb-3">
        <span className="block text-xs font-medium text-stone-600 mb-1">הערה</span>
        <textarea value={remark} onChange={(e) => setRemark(e.target.value)} rows={2} className="w-full px-2 py-2 text-sm border border-stone-300 rounded-md resize-y" dir="rtl" />
      </label>

      <div className="mb-3">
        <label className="flex items-center justify-center gap-2 w-full py-3 border-2 border-dashed border-stone-300 rounded-lg bg-white cursor-pointer text-sm text-stone-600">
      ...
      <button onClick={send} disabled={busy} className="w-full py-3 rounded-md font-bold text-white disabled:opacity-50" style={{ background: ARVO_DARK }}>{busy ? 'שולח...' : 'שלח הצעה ←'}</button>

...
// Phase 19 Stage B step 5 — interactive status badge.
// Phase 22 — add-project button + inline form (AddProjectForm client sub-component).
              ...
              {projects.map((p) => (
                <tr key={p.id} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                  <td className="py-3 px-2">
                    ...
                    {p.description_he ? (
                      <div className="text-xs text-stone-500 mt-1 line-clamp-1">{p.description_he}</div>
                    ) : null}
                  </td>
                  <td className="py-3 px-2">
                    <ProjectStatusBadge projectId={p.id} customerId={customerId} currentStatus={p.status} />
                  </td>
                  <td className="py-3 px-2 text-stone-700">{formatDate(p.inquiry_date)}</td>

...
// Phase 19 Stage B — Searchable customer list with click-to-navigate rows.
// Used by /customers index page.
      ...
      <div className="mb-4">
        ...
        {query && (
          <div className="text-xs text-stone-500 mt-1 px-1">
            מציג {filtered.length} מתוך {customers.length}
          </div>
        )}
      </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((c) => (
                  <tr
                    key={c.id}
                    className="border-b border-stone-100 hover:bg-blue-50 transition-colors"
                  >
                    <td className="py-3 px-3">
                      ...
                      {c.name_en && (
                        <div className="text-xs text-stone-500 mt-0.5" dir="ltr">{c.name_en}</div>
                      )}
                    </td>
